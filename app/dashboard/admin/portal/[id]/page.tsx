...
export default function ProjectReportPage() {
  ...
  const [filterDate, setFilterDate] = useState("") // Estado do Filtro
  ...
  return (
    <main className="min-h-screen w-full bg-[#020617] text-white pt-10 md:pt-20 px-6 pb-32">
        ...
        {entries.length > 0 ? (
          <div className="grid grid-cols-3 gap-3">
            {entries.map((entry) => (
              ...
              >
                {/* 1. Bloco Superior: Tag do Motorista + Horário no Canto */}
                <div className="flex justify-between items-start w-full">
                  <div className="bg-slate-100 px-2.5 py-1 rounded-lg inline-block shadow-sm">
                    <span className="uppercase font-black tracking-wider leading-none block" style={{ fontSize: '8px', color: '#64748b' }}>
                      👤 {entry.driver_name.split(' ')[0]}
                    </span>
                  </div>
                  
                  {/* Horário Alinhado ao Canto Superior Direito */}
                  <span className="font-bold leading-none block mt-1" style={{ fontSize: '9px', color: '#94a3b8' }}>
      ...
      {selectedEntry && (
        <div 
          className="fixed inset-0 z-[500] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 md:p-10 animate-in fade-in duration-300 overflow-y-auto"
          onClick={() => setSelectedEntry(null)}
        >
          <div 
            className="glass w-full max-w-lg p-6 md:p-8 rounded-[40px] border border-white/10 shadow-2xl relative text-white my-8 transition-all duration-300 scale-100"
          ...
          >
            ...
            <div className="space-y-6 text-left">
              ...
              {/* Foto do Comprovante Carimbada */}
              <div className="relative group rounded-[32px] overflow-hidden border border-white/10">
              ...
              <div className="space-y-3 pt-2">
                <a 
                  ...
                  download={`comprovante-${selectedEntry.vehicle_plate}.jpg`}
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="w-full bg-orange-500 hover:bg-orange-600 text-black font-black uppercase tracking-[2px] py-5 rounded-2xl transition-all active:scale-95 shadow-xl flex items-center justify-center gap-2 text-xs"
                >
                  📥 Baixar Comprovante
                </a>
                <button 
                  onClick={() => setSelectedEntry(null)} 
                  className="w-full bg-white/5 hover:bg-white/10 text-white/60 font-black uppercase tracking-[2px] py-5 rounded-2xl transition-all active:scale-95 border border-white/10 text-xs"
                >
                  Fechar Resumo
                </button>
              </div>
